...
// send_packet sends a single SYN packet to `dst_ip`:`dst_port` from `src_ip`.
// This function does not handle receiving packets.
async fn send_packet(
    ...
    let dst_ipv4: Ipv4Addr = match Ipv4Addr::from_str(dst_ip.to_string().as_str()) {
        Ok(addr) => addr,
        Err(_) => return,
    };

    let src_ipv4: Ipv4Addr = match Ipv4Addr::from_str(src_ip.to_string().as_str()) {
        Ok(addr) => addr,
        Err(_) => return,
    };

    let mut pkt_buf: [u8; 36] = [0; 36];
    let mut packet = MutableTcpPacket::new(&mut pkt_buf).unwrap();
    ...
    let channel_type = pnet_transport::TransportChannelType::Layer4(
        pnet_transport::TransportProtocol::Ipv4(IpNextHeaderProtocols::Tcp),
    );
    ...
    match tx.send_to(packet, dst_ip.to_owned()) {
        Ok(_) => (),
        Err(_) => (),
    }
}

// recieve_packets initiates a listener on `intf`, listening on `dst_port` for any responses from `src_ip`.
fn recieve_packets(intf: Arc<NetworkInterface>, dst_port: u16, src_ip: Arc<IpAddr>) {
    let (_, mut rx) = match pnet_datalink::channel(&intf, Default::default()) {
    ...
    loop {
        ...
        if ipv4.get_next_level_protocol() == pnet::packet::ip::IpNextHeaderProtocols::Tcp {
            ...
            if tcp.get_destination() == dst_port
                && ipv4.get_source().to_string() == src_ip.to_string()
            {
                if tcp.get_flags()
...
// collect_ports returns an iterator of all ports to be scanned.
// TODO: Add custom port selection.
fn collect_ports(all: bool) -> Box<dyn Iterator<Item = u16>> {
    if all {
        return Box::new((1..=u16::MAX).into_iter());
    }

    return Box::new(TOP_1000_PORTS.to_owned().into_iter());
}

// scan initiates a scanner on a single host, calling `scan_port` against all specified ports, and listening for responses.
async fn scan(
    ...
    let dst_ip = match IpAddr::from_str(dst_ip_str.as_str()) {
        Ok(addr) => addr,
        Err(_) => return,
    };

    let src_ip = match intf.ips.first() {
        Some(ip) => IpAddr::from(ip.ip()),
        None => return,
    };

    let intf_arc = Arc::new(intf.clone());
    let dst_ip_arc = Arc::new(dst_ip.clone());

    let recv_task = tokio::task::spawn(async move {
        recieve_packets(intf_arc.to_owned(), 12345, dst_ip_arc);
    });

    let ports = stream::iter(collect_ports(all_ports));
    ports
        .for_each_concurrent(concurrency, |port| {
            send_packet(intf, &src_ip, &dst_ip, port, delay)
        })
        .await;
...
async fn main() {
    ...
    let default_interface: Option<&NetworkInterface> = interfaces
        .iter()
        .find(|intf| intf.is_up() && !intf.is_loopback() && !intf.ips.is_empty());
    match default_interface {
        Some(intf) => {
            println!(
                "Using interface '{}' with IP address {}",
                intf.name,
                intf.ips.first().unwrap().ip()
            );
            println!("Scanning {}", args.target.to_string());

            scan(intf, args.target, false, 8, 100).await;
        }
        None => {
            println!("Could not find primary network interface, exiting...");
